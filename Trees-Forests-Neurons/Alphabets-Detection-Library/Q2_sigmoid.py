...
hidden_layers = [100]			# The structure of neural network 
alpha = 0.1/M 					# Learning rate of the model
...
def neural_overall(train_x, thetha, hidden_layers, r, count):
	...
	# Considering the hidden_layers being passed as a queue. So, look at the top-most element
		...
		thetha_new=thetha[count]
		train_x = [1.] + (train_x)
		output = [sigmoid(np.dot(np.array(train_x),np.array(thetha_new[i][0:len(train_x)]))) for i in range(units)]
...
def neural_outlist(train_x, thetha, hidden_layers, r, count, output, n):
	...
	# Considering the hidden_layers being passed as a queue. So, look at the top-most element
		...
		thetha_new=thetha[count]
		train_x = [1.] + (train_x)
		output_temp = [sigmoid(np.dot(np.array(train_x),np.array(thetha_new[i][0:len(train_x)]))) for i in range(units)]	
...
def neural_getdelta(thetha, hidden_layers, r, output, train_y, count, delta, maxVal, last_count):	
	...
	if (count==0):
		...
		delta[0,0:r] = (train_y - outlist) * (outlist) * (1-outlist)
	...
	# Using Backpropagation algorithm
		...
		delta_temp = [np.sum((delta_down)*(np.array(thetha[-count][:,j+1])[0:last_count])*((output[-(1+count)][j])*(1-output[-(1+count)][j]))) for j in range(hidden_layers[-count])]
...
def batch_gradient(thetha, hidden_layers, train_x, train_y, r, b, M, alpha, n):
	...
	return (thetha - (alpha * sum(gradlist)))
...
#		print(J_new)
	...
	if (J_new < 0.1 and abs(J_new - J_old)<conv) or (count>200):
		break
	else:
		J_old = J_new
