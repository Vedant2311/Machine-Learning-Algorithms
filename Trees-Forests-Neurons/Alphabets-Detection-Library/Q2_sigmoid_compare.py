...
def neural_getdelta(thetha, hidden_layers, r, output, train_y, count, delta, maxVal, last_count):	
	...
	# Using Backpropagation algorithm
		...
		delta_temp = [np.sum(np.multiply((delta_down),(np.array(thetha[-count][:,j+1])[0:last_count]))*((output[-(1+count)][j])*(1-output[-(1+count)][j]))) for j in range(hidden_layers[-count])]
